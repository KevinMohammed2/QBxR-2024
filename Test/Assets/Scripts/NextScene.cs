...
public class NewScene : MonoBehaviour
{
    ...
    {
        // Check for collisions with the ground, WR, or DB
        if (!playEnded && (collision.gameObject.CompareTag("FootballField") ||
                           collision.gameObject.CompareTag("BlackTeam") ||
                           collision.gameObject.CompareTag("GoldTeam")))
        {
            playEnded = true; // Mark the play as ended to avoid repeated triggers

            // Display the result
            resultPanel.SetActive(true);
            resultText.text = collision.gameObject.CompareTag("BlackTeam") ? "Complete" : "Incomplete";
