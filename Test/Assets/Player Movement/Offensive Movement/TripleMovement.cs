...
public class TripleMovement : MonoBehaviour, IRoute
{
  ...
  public float angleThirdDirection = 0f;   // Angle for the third direction
  ...
  public float distanceThirdPhase = 7f;    // Distance to cover in the third phase (7 yards)
  ...
  private int movementPhase = 0;           // Current movement phase (0, 1, or 2)
  ...
  void Update()
  {
    ...
    if (movementStarted)
    {
      ...
      if (coverageActive && !movementComplete)
      {
        ...
        float currentDistance = movementPhase == 0 ? distanceFirstPhase :
                                movementPhase == 1 ? distanceSecondPhase : distanceThirdPhase;
        float coverageAngle = movementPhase == 0 ? angleFirstDirection :
                              movementPhase == 1 ? angleSecondDirection : angleThirdDirection;
        ...
        else
        {
          ...
          // If the third phase is completed, mark the movement as complete
          if (movementPhase > 2)
          {
